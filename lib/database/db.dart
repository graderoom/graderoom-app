...
class DB {
  ...
  static StoreRef _store = StoreRef<String, dynamic>.main();
  ...
  static Future<void> _openDB() async {
    ...
    await courseCache;
    await settingsCache;
    toastDebug("Opened new database");
  }
  ...
  static Future<void> _fixCourses() async {
    ...
    for (var i = 0; i < courses.length; i++) {
      ...
      for (var j = 0; j < currentKeys.length; j++) {
        ...
        toastDebug("Deleted " + currentKeys[j] + " from course " + j.toString());
      }
      for (var j = 0; j < keys.length; j++) {
        if (currentKeys.contains(keys[j])) continue;
        course[keys[j]] = null;
        toastDebug("Added " + keys[j] + " to course " + j.toString());
      }
    }
  ...
  static Future<void> _fixSettings() async {
    ...
    for (int j = 0; j < currentKeys.length; j++) {
      ...
      toastDebug("Deleted setting: " + currentKeys[j]);
    }
    for (int j = 0; j < keys.length; j++) {
      if (currentKeys.contains(keys[j])) continue;
      settings[keys[j]] = null;
      toastDebug("Added setting: " + keys[j]);
    }
  ...
  static Future<void> _fixLocal() async {
    ...
    for (int j = 0; j < currentKeys.length; j++) {
      ...
      toastDebug("Deleted local: " + currentKeys[j]);
    }
    for (int j = 0; j < keys.length; j++) {
      if (currentKeys.contains(keys[j])) continue;
      local[keys[j]] = null;
      toastDebug("Added local: " + keys[j]);
    }
  ...
  static Future<void> initCourseCache() async {
    ...
    for (Map<String, dynamic> course in courseList) {
      _courseCache!.add(Course.fromMapOrResponse({...course}).toMap());
    }

    toastDebug("Initialized Course Cache");
  }
  ...
  static Future<void> _initSettingsCache() async {
    ...
    toastDebug("Initialized Settings Cache");
  ...
  static Future<void> _initLocalCache() async {
    ...
    toastDebug("Initialized Local Cache");
    ...
    if (DB.getLocal("stayLoggedIn") == null) {
      await DB.setLocal("stayLoggedIn", true);
    }
  }
  ...
  static Future<void> _addCourse(Course newCourse) async {
  ...
  static Future<void> writeCoursesFromString(String courseString) async {
    ...
    if (numCourses != null && numCourses! > 0 && courses.length > 0) {
      await _clearCourses();
    }
    for (Course course in courses) {
      await _addCourse(course);
  ...
  static Future<void> setLocal(String key, dynamic value) async {
    ...
    toastDebug("Saved local setting: " + key + " | " + value.toString());
  }
